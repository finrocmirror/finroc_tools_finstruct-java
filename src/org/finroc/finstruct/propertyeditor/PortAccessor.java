...
public class PortAccessor<T extends TypedObject> implements PropertyAccessor<T>, PortListener, CCPortListener {
    ...
    public T get() throws Exception {
        ...
        if (wrapped instanceof PortBase) {
            ...
            PortData pd = (PortData)pb.getLockedUnsafeRaw();
            T result = ObjectCloner.clone((T)pd);
            pd.getManager().releaseLock();
            return result;
        } else {
            CCPortBase cpb = (CCPortBase)wrapped;
            CCPortData pd = cpb.getAutoLockedRaw();
            T result = ObjectCloner.clone((T)pd);
    ...
    public void set(T newValue) throws Exception {
        ...
        if (ap instanceof CCPortBase) {
            if (ap.getFlag(CoreFlags.NETWORK_ELEMENT)) {
                CCInterThreadContainer<?> c = ThreadLocalCache.get().getUnusedInterThreadBuffer(newValue.getType());
                c.assign((CCPortData)newValue);
                RemoteRuntime.find(ap).getAdminInterface().setRemotePortValue(ap.asNetPort(), c);
            } else {
                CCPortDataContainer<?> c = ThreadLocalCache.get().getUnusedBuffer(newValue.getType());
                c.assign((CCPortData)newValue);
            ...
            PortData result = TypedObjectCloner.cloneHelper(pd, new PortDataManager(pd.getType(), null).getData());
            if (ap.getFlag(CoreFlags.NETWORK_ELEMENT)) {
